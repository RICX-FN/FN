document.addEventListener("DOMContentLoaded", () => {
    ...
    const nomeInput = document.getElementById("nome-reg-cliente");
    const emailInput = document.getElementById("email-reg-cliente");
    const senhaInput = document.getElementById("password-reg-cliente");
    ...
    async function checkEmailInDatabase(email) {
        try {
            ...
            return data.existe; // Supondo que o backend retorna { existe: true/false }
        } catch (error) {
        ...
        if (isNomeValid && isEmailValid && isSenhaValid) {
            try {
                ...
                if (response.ok) {
                    ...
                    const errorData = await response.json(); // Obtém os dados de erro do backend
                    const errorMessage = errorData.erro || "Erro ao registrar."; // Exibe a mensagem do backend ou uma mensagem genérica
                    showError(emailInput, errorMessage); // Exibe a mensagem de erro no campo de email
                }
            } catch (error) {
